...
## 5.5 扫描投影参数标定(min_height / max_height / range_min)

除了 6 个外参,`/scan` 还由 `pointcloud_to_laserscan` 的**高度/距离裁剪**决定 —— **这几个参数也算标定的一部分**,现已提成 launch arg(本 launch 与 `mapping_test.launch` / `navigation.launch` 三处一致),现场可换值重启调:

```bash
roslaunch neupan_ros calib_lidar.launch min_height:=-0.05 range_min:=0.2
```

### 坐标系约定(关键)
- 三个值都相对 **base_link(轮轴中心)** 量,base_link 在地面上方 **0.181m**。
- **高度**:`离地高 = 参数值 + 0.181`(地面在 base_link 系里是 `z = -0.181`)。
- `range_min` 是**水平距离** `√(x²+y²)`,从 base_link 轴心量,**不含高度**;圈内的点直接丢弃(管"多近"),与 `min_height` 管"多低"正交。

### 三个参数

| 参数 | 作用 | 怎么定 | 默认 |
|------|------|--------|------|
| `min_height` | 切片**下沿**:滤地面 + 车自身结构 | 量"雷达能看到的最高自身结构(板子/支架)相对 base_link 高度" + 3~5cm 余量 | **0.0** |
| `max_height` | 切片**上沿**:忽略天花板/可钻过的悬挂物 | 环境选择,一般不动 | 1.0 |
| `range_min` | 水平**近距**裁剪:屏蔽车体一圈 | 量"轴心到车体最外缘水平距离" + 余量;≈ Scout 半宽 | 0.3 |

### 本车结论:雷达零自检测 → min_height 自由

经 SolidWorks 射线核实,**本车雷达扫不到任何自身部位(含板子/支架)**。因此:
- `min_height` 不再被"自检测"约束,**只受地面(z=-0.181)限制** → 默认设 **0.0**(离地 0.18m,比旧的 0.15/离地 0.33m 多看一截矮障碍)。想看更矮可降到 `-0.05 ~ -0.1`。
- `range_min` 也不必为挡车体而设大,保持 **0.3**(≈ Scout 半宽 0.29)即可。

### RViz 调法(零自检测下,盯地面而非车身)
开 `/scan` + 车模型,往下调 `min_height`:
- 远处(约 **4.4m 外**,-7° 光束掠地处)冒出地面噪点 → 退回一点;
- 没冒 → 可继续往下,直到想要的最矮障碍高度。

> ⭐ **交接物**:`min_height`(及 `range_min`)调定后,**三个 launch 要用同一组值**。默认已统一为 `0.0 / 1.0 / 0.3`;若你改了,记得三处一起改(或都用命令行传同一组)。
